    ...
    <div className="bg-white text-ink-900 pb-24 sm:pb-0">
      ...
      <section className="relative bg-gradient-to-b from-ink-900 via-ink-800 to-ink-700 text-white overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl px-5 sm:px-6 py-16 sm:py-24">
          <p className="text-brand-500 text-xs sm:text-sm font-bold uppercase tracking-[0.25em]">
          ...
          <p className="mt-8 max-w-xl border-l-2 border-brand-500 pl-4 text-sm text-white/60 leading-relaxed">
            {content.descarte}
          </p>
        </div>
      </section>
      ...
      <section aria-label="Credenciales" className="border-b border-line bg-white">
        <div className="mx-auto max-w-4xl px-5 sm:px-6 py-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
          {trustChips.map((chip) => (
            <div key={chip.label} className="flex items-center gap-3 text-sm font-bold text-ink-700">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-panel text-brand-600">
                {chip.icon}
              </span>
              {chip.label}
            </div>
        ...
        <div className="mx-auto max-w-4xl px-5 sm:px-6">
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-3">
            {content.pasos.map((paso, i) => (
              ...
              >
                <span className="flex h-11 w-11 items-center justify-center rounded-full bg-ink-700 font-serif text-xl font-semibold text-white">
                  {i + 1}
                </span>
        ...
        <div className="mx-auto max-w-4xl px-5 sm:px-6">
          ...
          <div className="mt-8 grid gap-4">
            {content.experiencia.items.map((item, i) => (
              ...
              >
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-panel text-brand-600">
          ...
          <p className="mt-5 rounded-lg border border-brand-500/40 bg-brand-100/50 px-4 py-3 text-xs text-ink-800 leading-relaxed">
      ...
      <section className="bg-ink-900 py-14 sm:py-20 text-white">
        <div className="mx-auto max-w-4xl px-5 sm:px-6 grid gap-10 sm:grid-cols-2">
          <div data-reveal>
            <p className="font-sans text-sm font-semibold text-brand-500 tracking-wide">03/</p>
            <h2 className="mt-1 font-serif text-2xl sm:text-3xl font-semibold">Quién responde su consulta</h2>
          ...
          <div data-reveal style={{ transitionDelay: "70ms" }} className="rounded-xl border border-white/15 bg-white/5 p-6">
            <div className="flex items-center gap-3">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="h-6 w-6 text-brand-500" aria-hidden="true">
      ...
      <section className="bg-surface py-14 sm:py-20">
        <div className="mx-auto max-w-4xl px-5 sm:px-6">
          <div data-reveal>
            <p className="font-sans text-sm font-semibold text-brand-600 tracking-wide">04/</p>
            <h2 className="mt-1 font-serif text-2xl sm:text-3xl font-semibold text-ink-900">
          ...
          <div className="mt-8 space-y-3" data-reveal>
            {content.faq.map((f) => (
              <details key={f.pregunta} className="group rounded-xl border border-line bg-white px-5 py-4 open:shadow-[0_4px_6px_rgba(15,23,42,0.06)]">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-serif text-base sm:text-lg font-semibold text-ink-900 min-h-[44px]">
                  {f.pregunta}
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-5 w-5 shrink-0 text-brand-600 transition-transform duration-200 group-open:rotate-180" aria-hidden="true">
              ...
              <>
                ...
                <form onSubmit={handleSubmit} className="mt-6 grid gap-5">
                  ...
                  <div>
                    ...
                    <textarea id="c-caso" name="message" required rows={4} className="mt-1.5 w-full rounded-md border border-line bg-white px-4 py-3 text-ink-900 focus:outline-none focus:ring-2 focus:ring-ink-600 focus:border-ink-600 transition-shadow" />
                  </div>
                  ...
                  <button
                    ...
                    className="rounded-lg bg-ink-700 px-8 py-4 text-base font-bold text-white shadow-[0_4px_14px_rgba(19,19,19,0.25)] hover:bg-ink-600 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 disabled:opacity-60 disabled:hover:translate-y-0"
                  >
